...
class TestUsers:
    ...
    def test_register_user_with_invalid_field_returns_400(
        self, client, field, invalid_value
    ):
    ...
    def test_register_user_with_duplicate_field_returns_400(
            self, client, user, field):
    ...
    def test_user_profile_available_and_returns_correct_data(
        ...
        assert json_data.get(
            "first_name") == user.first_name, "Неверный first_name."
        assert json_data.get(
            "last_name") == user.last_name, "Неверный last_name."
        assert (
            json_data.get("is_subscribed") is False
    ...
    def test_delete_avatar_successfully(self, auth_client, tmp_path):
        ...
        assert (
            response.json()["avatar"] is None
        ), "После удаления аватара это поле должно быть пустым."

    @pytest.mark.parametrize(
        "payload", [{}, {"avatar": ""}, {"avatar": "invalid_format"}]
    )
    ...
    def test_token_login_successfully(self, client, user):
        ...
        response = client.post(
            USER_LOGIN_URL,
            data={"email": user.email, "password": user.plaintext_password},
        )
        assert response.status_code == HTTPStatus.OK, (
