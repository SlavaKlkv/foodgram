...
class TestRecipes:
    ...
    def test_create_recipe_returns_201(
        ...
        assert Recipe.objects.count() == recipes_count_before + 1, (
        ...
        assert {tag['id'] for tag in json_data['tags']} == set(
            recipe_data['tags']), (
        ...
        returned = {
            (item['id'], item['amount']) for item in json_data['ingredients']
        }
        expected = {
            (
                item['id'], item['amount']
    ...
    def test_create_and_update_recipe_with_invalid_field_returns_400(
        ...
        url = RECIPES_URL if method == 'post' \
            else RECIPE_DETAIL_URL.format(id=recipe.id)
    ...
    def test_update_recipe_successfully(
        ...
        assert json_data.get('ingredients')[0]['id'] == \
        ...
        assert json_data.get('ingredients')[0]['amount'] == \
    ...
    def test_not_author_cant_update_or_delete_recipe(
        ...
        и поля не меняются.
        """
        url = RECIPE_DETAIL_URL.format(
            id=recipe_with_ingredients_and_tags.id
        )
        ...
        assert sorted([t['id'] for t in after_response_json['tags']]) == \
               sorted([t['id'] for t in before_response_json[
                   'tags']]), 'Список тегов изменился!'